...
export default function get_estimate() {
  ...
  function makeSymptonImg(data: Data<UserData>) {
    if (data.img === undefined) return;
    imgBtn.style.display = "none";
    commonMakeImg(data);
  }

  function removeAndMakeNewImage(data: Data<UserData>) {
    let imgBox = document.querySelector(".si-img-itemBox");
  ...
  function commonMakeImg(data: Data<UserData>) {
  ...
  async function delete_img_session(e: any) {
    try {
      ...
      let result = await fetch("http://localhost:3000/web/delete/session/img", fetchObj);
  ...
  async function fetchImage(url: string, data: string[]) {
  ...
  window.onpageshow = function (event: any) {
    if (event.persisted || (window.performance && window.performance.navigation.type == 2)) {
      ...
      (async () => {
        try {
          let result = await fetch("http://localhost:3000/web/users/session");
          if (result.status === 200 || 201) {
            let response = await result.json();
